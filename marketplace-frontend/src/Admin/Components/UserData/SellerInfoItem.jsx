...
const SellerInfoItem = ({id }) => {
    ...
    const updateUser = async () => {
        try {
            await axios.put(`http://localhost:8080/api/v1/sellers/${id}`, userData, {
                withCredentials: true
            });
            toast.success("Seller updated successfully", { autoClose: 500, theme: 'colored' });
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await axios.get(`http://localhost:8080/api/v1/sellers/${id}`, {
                    withCredentials: true
                });
    ...
    const deleteAccount = async () => {
        try {
            const deleteUser = await axios.delete(`http://localhost:8080/api/v1/sellers/${id}`, {
                withCredentials: true
            });
    ...
    return (
        <Container sx={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', marginBottom: 10 }}>
            <Typography variant='h6' fontWeight="bold" sx={{ margin: '20px 0' }}>Seller Details</Typography>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', margin: "25px 0", width: '100%' }}>
                <Button variant='contained' color='primary' onClick={() => setOpenUpdateDialog(true)}>Update Seler</Button>
            ...
            >
                <DialogContent sx={{ width: { xs: 280, md: 350, xl: 400 } }}>
                    <DialogContentText style={{ textAlign: 'center' }} id="alert-dialog-slide-description">
                        <Typography variant='body1'> Are you sure you want to update the seler's details?</Typography>
                    </DialogContentText>
                </DialogContent>
